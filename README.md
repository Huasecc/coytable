...
- **Inline Cell Editing** — Click any text cell to edit — `Enter` confirm, `Esc` cancel, `Tab` navigate
...
- **Pan & Zoom** — Spacebar + drag to pan; Ctrl+wheel to zoom (25%–400%)
...
- **Alignment** — Set horizontal text alignment: left / center / right
...
- **Column Width / Row Height** — Set fixed values or fill remaining space
...
#### 🎯 Header Style
- **Background / Text Color** — Customize header appearance
- **Font Weight** — Regular or Semi Bold

#### 📦 Cell Style
- **Background / Text Color** — Body cell appearance
...
#### 🎨 Preset Styles
8 one-click themes: Default, Dark, Blue Business, Warm Report, Green Fresh, Pink Warm, Purple Elegant, Gray Minimal

#### 📋 Preset Data
12 data templates: User Info, Product List, Project Board, Task Tracker, Customer Management, Order Management, Bug Tracker, Finance Ledger, Meeting Notes, Course Ratings, Design Tasks, Recruitment
...
- **Checkbox** — Square (checked/unchecked), toggle on click
...
- **Insert above** — Click the handle → popup → **Insert Above**
- **Insert below** — Click the handle → popup → **Insert Below**
- **Duplicate** — Click the handle → popup → **Copy**
- **Delete** — Click the handle → popup → **Trash** (min 1 row)
...
- **Insert left** — Click the handle → popup → **Insert Left**
- **Insert right** — Click the handle → popup → **Insert Right**
- **Duplicate** — Click the handle → popup → **Copy**
- **Delete** — Click the handle → popup → **Trash** (min 1 column)
- **Width** — Click the handle → enter pixel width in the popup
- **Horizontal align** — Click the handle → popup → left / center / right
- **Change type** — Click the handle → select cell type icon
...
2. A **Re-edit** bar appears at the bottom of the plugin
...
- `Esc` — Cell editing — Cancel edit
- `Tab` — Cell editing — Next cell
- `Shift` + `Tab` — Cell editing — Previous cell
- `Space` — Preview area — Hold + drag to pan
- `Ctrl` + `Wheel` — Preview area — Zoom in/out
...
- **嵌入编辑** — 点击文本单元格直接编辑，`Enter` 确认、`Esc` 取消、`Tab` 跳格
- **拖拽排序** — 拖拽行/列手柄（⠿）自由调整顺序
- **花式列类型** — 文本、单选、复选、序号、图片、进度条、标签、操作按钮
...
- **中英双语** — 底部栏一键切换中文/英文界面
- **明暗主题** — 一键切换亮色/暗色模式
- **回传编辑** — 选中画布上的已有表格，一键载入继续修改
- **斑马纹** — 开启后交替行背景色，表格更清晰易读
- **8 套预设样式** — 默认、暗黑、蓝色商务、暖色报表、清新绿意等
- **平移缩放** — 空格+拖拽平移，Ctrl+滚轮缩放（25%–400%）
...
#### 🎨 预设样式
8 套一键主题：默认标准、暗黑、蓝色商务、暖色报表、清新绿意、粉色温馨、紫色典雅、灰色极简

#### 📋 预设数据
12 套快速填充模板：用户信息、商品清单、项目看板、任务追踪、客户管理、订单管理、Bug 追踪、财务流水、会议纪要、课程评分、设计任务、招聘管理
